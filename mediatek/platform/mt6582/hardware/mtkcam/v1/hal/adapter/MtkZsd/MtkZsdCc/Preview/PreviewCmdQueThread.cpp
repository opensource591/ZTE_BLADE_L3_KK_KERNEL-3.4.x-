    ...
    */
    ...
    if (flag & eID_Pass2DISPO)
    {
        ...
        if (!isGenaric && capNode.subImgNode.getImgBuf() != 0) {
            ...
            {
                // 16 alignment 
                if((dispNode->getImgWidth() % 16) != 0)
                {
                  int temp = dispNode->getImgWidth() / 16;
                  uint32_t dispNodeImgWidth = 16* (temp+1);
                  
                  mConvertor.setSrc(dispNode->getImgFormat(),
                                   (unsigned char*)dispNode->getVirAddr(),
                                   dispNode->getBufSize(),
                                   dispNodeImgWidth , //16 alignment size
                                   dispNode->getImgHeight(),
                                   dispNode->getImgWidthStride(0),
                                   dispNode->getImgWidthStride(1),
                                   dispNode->getImgWidthStride(2));
                ...
                // 16 alignment
                if((vidoNode->getImgWidth() % 16) != 0)
                {
                  int temp = vidoNode->getImgWidth() / 16;
                  uint32_t vidoNodeImgWidth = 16* (temp+1);

                  mConvertor.setDst(vidoNode->getImgFormat(),
                                   (unsigned char*)vidoNode->getVirAddr(),
                                   vidoNode->getBufSize(),
                                   vidoNodeImgWidth, //16 alignment size
