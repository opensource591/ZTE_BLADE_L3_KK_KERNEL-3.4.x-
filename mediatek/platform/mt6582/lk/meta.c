...
{  
  ...
  if (g_boot_arg->maggic_number == BOOT_ARGUMENT_MAGIC)
  {
    ...
    else if (mode == FACTORY_BOOT)
    {
      ...
      return TRUE;
    }else if(mtk_detect_key(MT65XX_FACTORY_KEY) && mtk_detect_key(MT65XX_RECOVERY_KEY))
	{
		g_boot_mode = META_BOOT;
		g_boot_arg->maggic_number = BOOT_ARGUMENT_MAGIC;
		g_boot_arg->meta_com_type = META_USB_COM;
		//g_boot_arg->meta_com_type = META_UART_COM;  //for uart 
		g_boot_arg->meta_com_id=1;

		printf("META Detect key\n");
		printf("Enable META mode\n");
		return TRUE;
