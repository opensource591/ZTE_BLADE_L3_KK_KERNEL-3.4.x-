...
static void init_lpddr2(EMI_SETTINGS *emi_setting) 
{
	...
	*((volatile unsigned *)(DDRPHY_BASE + 0x01e4)) = 0x00001100;
	...
	{//only for POP LP2 PL017-25, use all-bank refresh only 
		// Edward : add two rank enable here.
		if ( *(volatile unsigned *)(EMI_CONA)& 0x20000) {
			*((V_UINT32P)(DRAMC0_BASE + 0x0110)) = 0x00112311;	
			*((V_UINT32P)(DDRPHY_BASE + 0x0110)) = 0x00112311;
		} else {
			*((V_UINT32P)(DRAMC0_BASE + 0x0110)) = 0x00112310;	// Edward : per bank refresh enable here. 
			*((V_UINT32P)(DDRPHY_BASE + 0x0110)) = 0x00112310;
		}
	}
	else
	{
...
static void init_ddr3(EMI_SETTINGS *emi_setting) 
{
	...
	*((volatile unsigned int *)(EMI_BASE + 0x0158))=0x1f011f00;// ???????????????????????0x08090800; 
	// RFF)_PBC_MASK
	if((!DRAM_IS_2_RANK_MODE()) && (*((volatile unsigned int*)(EMI_CONA)) & 0xC0C0))
	{
	...
	*((volatile unsigned int *)(DDRPHY_BASE + 0x0000)) = emi_setting->DRAMC_ACTIM_VAL;//0x33584562;		// Edward : TWTR[11:8]=5h,	TRC[7:4]=6, TRAS[3:0]=2h
	...
	// Edward : AC_TIME_0.5T control (new for 6582)
	*((volatile unsigned int *)(DRAMC0_BASE + 0x01f8)) = emi_setting->DRAMC_ACTIM05T_VAL;//0x05002000;  //TWTR_M[26]=1, TRTW[24]=1, TFAW[13]=1
	*((volatile unsigned int *)(DDRPHY_BASE + 0x01f8)) = emi_setting->DRAMC_ACTIM05T_VAL;//0x05002000;   
