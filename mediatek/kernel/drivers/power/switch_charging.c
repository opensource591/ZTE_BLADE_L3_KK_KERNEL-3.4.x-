...
static kal_uint32 charging_full_check_cc(void)
{
	kal_uint32 status;
//	printk("charging_full_check-----BMT_status.ICharging:%d--BMT_status.bat_vol:%d\n",BMT_status.ICharging,BMT_status.bat_vol);

	battery_charging_control(CHARGING_CMD_GET_CHARGING_STATUS,&status);

	if((BMT_status.ICharging < 190&&BMT_status.bat_vol>4300)||(status == KAL_TRUE))
	{
			status = KAL_TRUE;
	}
	else
...
static kal_uint32 charging_full_check_cv(void)
{
	kal_uint32 status ;
//	printk("charging_full_check-----BMT_status.ICharging:%d--BMT_status.bat_vol:%d\n",BMT_status.ICharging,BMT_status.bat_vol);
#if 0
...
PMU_STATUS BAT_ConstantCurrentModeAction(void)
{
	...
	if(charging_full_check_cc() == KAL_TRUE)
	{
	    BMT_status.bat_charging_state = CHR_BATFULL;
		BMT_status.bat_full = KAL_TRUE;
		g_charging_full_reset_bat_meter = KAL_TRUE;
...
//    battery_charging_control(CHARGING_CMD_SET_CV_VOLTAGE,BATTERY_VOLT_04_300000_V);
//	battery_charging_control(CHARGING_CMD_SET_POWER_OFF,&charging_enable);
	battery_charging_control(CHARGING_CMD_ENABLE,&charging_enable);

	if(charging_full_check_cv() == KAL_FALSE)
    {
