...
void dod_init(void)
{
    ...
    #endif
	//add by lgw 
	static kal_int32 capacity_tolrance=35;
	if(bat_is_charger_exist() == KAL_TRUE)
		    capacity_tolrance=60;
	  else
	        capacity_tolrance=35;
	//end
	
    if(g_rtc_fg_soc >= gFG_capacity_by_v)
    {
        if(((g_rtc_fg_soc != 0) && ((g_rtc_fg_soc-gFG_capacity_by_v) < capacity_tolrance) &&(( gFG_capacity_by_v > CUST_POWERON_LOW_CAPACITY_TOLRANCE || bat_is_charger_exist() == KAL_TRUE)))
    ...
    {
    	if(((g_rtc_fg_soc != 0) && ((gFG_capacity_by_v-g_rtc_fg_soc) < capacity_tolrance) &&(( gFG_capacity_by_v > CUST_POWERON_LOW_CAPACITY_TOLRANCE || bat_is_charger_exist() == KAL_TRUE)))
			|| ((g_rtc_fg_soc != 0) &&(g_boot_reason == BR_WDT_BY_PASS_PWK || g_boot_reason == BR_WDT || g_boot_reason == BR_TOOL_BY_PASS_PWK || g_boot_reason == BR_2SEC_REBOOT || g_boot_mode == RECOVERY_BOOT)))
        {
            gFG_capacity_by_v = g_rtc_fg_soc;            
        }
